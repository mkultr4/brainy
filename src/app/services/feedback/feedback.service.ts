...
import { FeedbackWorkflowService } from '../../modules/feedback/services/feedback-workflow.service';
import { MessagingService } from '../messaging.service';

@Injectable()
export class FeedbackService  extends SocketService {
    ...
    constructor() {
      ...
      this._pieces = new BehaviorSubject<Piece[]>([])
      this._versions = new BehaviorSubject<Piece[]>([])
      ...
      this._feedbackWorkflowService = new FeedbackWorkflowService();
    ...
    loadPiecesById(id: string, empty = false, generateComments = false) {
      ...
      this.dataStore.pieces = Object.assign([], []);
        const observable = new Observable((observer) => {
            // Init data stores
            this.proccessRequest('findPiecesByCore', id).subscribe((piecesByCore) => {
                    // Init data stores
                    this.dataStore.pieces = [];
                    this.dataStore.versions = [];
                    const pieces = piecesByCore.filter(p => p.latest === true && p.show === true).map((p) => {
                        p.commentThreads = []
                        return p;
                    });
                    this.dataStore.pieces = pieces;
                    const versions = piecesByCore.filter(p => p.latest === false).map((p) => {
                        p.commentThreads = []
                        return p;
                    });
   ...
   */
    ...
    private generateDefualtCommentsPiece(piece: Piece) {
      ...
      console.log('generateDefualtCommentsPiece');
      let left = 50;
      let top = 30;
    ...
    */
    deleteFeedbackPiece(pieceId: string) {
      ...
      const observable = new Observable((observer) => {
        ...
        this.proccessRequest('deletePiece', pieceId).subscribe((piece) => {
          console.log(piece);
        });
        this.orderFeedbackPiece();
        this._pieces.next(Object.assign({}, this.dataStore).pieces);
     ...
     * @param pieces
     */
    updateOrderFeedbackPieces(pieces: Array<any>) {
      const observable = Observable.create(async (observer) => {
        let order = 1;
        for (const piece of pieces) {
          if(order === 1) {
            console.log(JSON.stringify(piece));
            await this.proccessRequest('updateCore', {id: piece.core.id, core: { thumbnail: piece.url}});
          }
          await this.proccessRequest('updatePiece', {id: piece.id, data: { order: order}});
          order ++;
        }
        observer.next(true);
      });
   ...
   * @param files
   */
    uploadImages(firstPiece: Piece, files: File[], user: User) {
  
      const observable = Observable.create(async (observer) => {
        const pieces = [];
        let count = 1;
        for (const file of files) {
            await this.proccessRequest('createPiece', { 
                core: { id: firstPiece.coreId },
                file: file,
                fileName: file.name,
                status: 1,
                type: 'images',
    ...
    uploadDocument(firstPiece: Piece, file: File, user: User) {
      const observable = Observable.create(async (observer) => {
        const pieces = [];
        await this.proccessRequest('createPiece', { 
            core: { id: firstPiece.coreId },
    ...
    * @param file
    */
    uploadDocumentPdf(firstPiece: Piece, file: File, user:User) {
      const observable = Observable.create(async (observer) => {
        const pieces = [];
     ...
     * @param sites
     */
    uploadSites(firstPiece: Piece, sites: Site[], user: User) {
      const observable = Observable.create((observer) => {
        const pieces = [];
        let count = 1;
     ...
     * @param file
     */
    uploadVideo(firstPiece: Piece, file: File, user: User) {
   ...
   */
     ...
     */
    changePieceImage(piece: Piece, file: File, user: User ,mantainComments: boolean = false) {
      ...
      const observable = Observable.create(async (observer) => {
        ...
        await this.proccessRequest('changePiece', { 
            piece: piece,
            file: file,
            fileName: file.name,
            user: user
     ...
     * @param mantainComments 
     */
    changePieceVideo(piece: Piece, file: File, user: User, mantainComments: boolean = false) {
    ...
    pieceSelected() {
      const observable = Observable.create(async (observer) => {
      let piece;
        await this.socket.on('selectPiece', (data) => {
          if (data.coreId === this.coreId) {
            piece = data;
            observer.next(piece);
          }
        });
