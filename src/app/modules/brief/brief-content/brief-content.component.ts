...
export class BriefContentComponent
  ...
  ngAfterContentInit() {
    ...
    // If has id
    if (this.id) {
      this._briefService.loadById(this.id, 'project', this.statusKey).subscribe(brief => {
        if (brief && brief.id) {
          // Load topics
          this._briefService.loadCategories(this.id, false, this.autoFillResponse, this.addGiveback, this.addGivebackResponse).subscribe();
          // Load invitations
